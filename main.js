...
class Opentherm extends utils.Adapter {
    ...
    async onReady() {
        ...
        if (useTCP) {
            ...
            try {
                // @ts-expect-error net.connect accepts a connect options object
    ...
    /**
     * Create a channel object, replacement for the deprecated createChannel()
     *
     * @param {string} id id of the channel, relative to the adapter namespace
    ...
    async doChannelCreate() {
        ...
        if (devData) {
            await this.doChannelCreateDev();
        }

        await this.createChannelObject('config', 'config');
        await this.createChannelObject('control', 'control');
        await this.createChannelObject('fault', 'faul');
        await this.createChannelObject('info', 'info');
        await this.createChannelObject('status', 'status');
    ...
    async doChannelCreateDev() {
        await this.setObjectNotExistsAsync('_Dev', {
            type: 'device',
            common: {
        ...
        await this.createChannelObject('_Dev.0', 'Read-Data || msgType : 0');
        await this.createChannelObject('_Dev.1', 'Write-Data || msgType : 1');
        await this.createChannelObject('_Dev.2', 'Read-Ack || msgType : 2');
        await this.createChannelObject('_Dev.3', 'Write-Ack || msgType : 3');
        await this.createChannelObject('_Dev.4', 'Data-Inv || msgType : 4');
        await this.createChannelObject('_Dev.5', 'Unk-DataId || msgType : 5');
        await this.createChannelObject('_Dev.6', '????????? || msgType : 6');
        await this.createChannelObject('_Dev.7', '????????? || msgType : 7');
        await this.createChannelObject('_Dev.8', '????????? || msgType : 8');
