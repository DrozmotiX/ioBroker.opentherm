...
class Opentherm extends utils.Adapter {
  ...
  async onReady() {
    ...
    if (useUSB) {
      ...
      try {
        ...
        this.log.info(
          "OpenTherm : Succesfully connected on : " + "/dev/ttyUSB0",
        );
  ...
  async create_state(state, name, value) {
    ...
    try {
      ...
      const writable =
        state_attr[name] !== undefined
          ? state_attr[name].write || false
          : false;
      const state_name =
        state_attr[name] !== undefined ? state_attr[name].name || name : name;
      const role =
        state_attr[name] !== undefined
          ? state_attr[name].role || "state"
          : "state";
      const type =
        state_attr[name] !== undefined
          ? state_attr[name].type || "mixed"
          : "mixed";
      const unit =
        state_attr[name] !== undefined ? state_attr[name].unit || "" : "";
