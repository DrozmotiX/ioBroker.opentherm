...
function translate_input(adapter,msg){
	...
	try {
		...
		adapter.log.warn(JSON.stringify(msg))

//		adapter.log.error(msgType)
		...
		if (-1 != Object.keys(Map).indexOf(dataId.toString()) ){
		  Map[dataId].forEach( function(f) {
			...
			const outcome = '{"msgraw : "' + msgraw + ', "DataId : "' + dataId + ', "msgType : "' + msgType + ', "|| Device : " + msgtopic +  " || Value : " + msgpayload}'

			adapter.log.info(outcome)
//			  createState("testOT." + msgtopic, function () {

			return outcome

			});
//			setState("testOT." + msgtopic, msgpayload, true);
