...
function checkinput(adapter,msg) {
	...
	if (DevLogging){adapter.log.info('Messagee raw ' + raw);}

	if (raw.length != 9 && devData) {
		if (DevLogging){adapter.log.error('Illegal message length ' + raw);}
	}
	...
	{
		if (DevLogging){adapter.log.error('Illegal message type ' + raw);}
		checker = false;
	}
	...
	{
		if (DevLogging){adapter.log.error('Illegal message content ' + raw);}
		checker = false;
	...
	{
		if (DevLogging){adapter.log.error('Illegal message parity ' + raw);}
		checker = false;
	}
	...
	if (DevLogging){adapter.log.info('Bulding JSON object with : ' + '{"msgType" : "' + msgtype + '", "dataid" : "' + dataid + '", "datavalue" : "' + datavalue + '", "payload" : "' + raw + '"}');}
